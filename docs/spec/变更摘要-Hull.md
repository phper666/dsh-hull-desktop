...
- 内容：**D4 决策修订**——升级 UI 从独立 upgrade 视图并入设置页升级区块（删独立视图）；view 机制 4→3 态（去 placeholder:upgrade）；导航菜单去掉「升级」项，排序 = dsh web / 任务看板 / 设置（设置恒最后）；左下角状态区新增「Hull 版本」行（hull:status payload 加 hullVersion = app.getVersion()）
- 决策：D4 修订（升级并入设置）/ nav 排序（去升级，设置最后）/ Hull 版本号显示（2026-08-22 用户目验反馈）
- 红线：CON-R001~R005 全部不破（升级编排层零改动；官方 view 零注入不变；settings 走 hull:setSettings）
- 影响：契约 S3（v0.3→v0.4 UI 载体改并入设置）/ S6（v0.4→v0.5 设置含升级区块+nav 排序）/ 方案 §4.1/§4.5/§6/§8 修订
- 状态：代码已实现（commit e825ef1，测试 467+8+12 全绿），文档同步中
