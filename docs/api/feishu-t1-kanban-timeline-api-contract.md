...
数据源 = 当前看板全部任务（含子任务；不含归档区任务——归档活动随归档区展示，不进主时间线。**实现补充约定**：共识 §12 数据范围为「当前看板全部活动」，归档排除为本契约实现细化，待回写共识留痕）。每个任务派生活动条目：
...
### 6. 消毒与渲染管线（CON-R-editor-002 适用 / E1 对齐）

> T1 新增的 renderTimeline/renderCalendar 是新的用户内容进 DOM 表面，消毒规则显式约定如下；违反即契约违约。

| 内容 | 渲染管线 | 依据 |
|---|---|---|
| 时间线评论条目 content | **markdown-it + DOMPurify**（与 E1 detail 渲染同管线——E1 升级后评论为 Markdown，两视图渲染必须一致） | CON-R-editor-002 + E1 契约 FE-2 |
| 时间线 execution/system 条目 content、command 摘要 | `esc()` 纯文本（非用户 Markdown） | E1 契约 FE-2 |
| 任务标题/标签/assignee 等结构化字段（时间线+日历） | `esc()` 纯文本 | M2 既有模式 |
| 日历格内任务条目 | `esc()` 纯文本（仅标题级信息，无 Markdown 渲染面） | 同上 |

- **禁止裸插**：新增渲染站点不允许出现未消毒用户内容直插 innerHTML 的路径；站点清单并入 E1 Q-043 审计清单维护。
- XSS 场景见联调表 TL7。
...
| TL7 | 评论 XSS 不执行 | 评论 content 注入 `<img src=x onerror=alert(1)>` 与 `[x](javascript:alert(1))` | 渲染时间线视图 | 载荷不执行：onerror 移除、javascript: href 清除（markdown-it+DOMPurify 管线，与 E1 detail 同管线）；文本可见 | DOM 断言无事件属性/危险 href | T1-②/CON-R-editor-002 |
...
| CAL12 | 周视图区间跨周边界 | 任务 startDate=周二 + dueDate=下周三（跨周） | 周视图渲染两周 | 第 1 周条带覆盖周二→周末，第 2 周条带覆盖周一→周三；按日遍历逐格连续不跳格 | 两周条带覆盖断言 | T1-③/Q-054 |
...
| PERF4 | 加载更多第二页不重不漏 | ≥1000 卡时间线分页 | 首屏渲染 → 点"加载更多"翻至第二页 | 新增条目 id 与首屏无重复、与全量集合比对无遗漏；总条目数 = 前两页之和 | id 集合 diff 断言 | T1-④/CON-R-timeline-006 |
...
| **E1×T1 评论渲染对齐** | 时间线视图评论条目与 E1 detail 详情共用 markdown-it + DOMPurify 渲染管线（CON-R-editor-002 适用，见本契约 §6）；管线实现以 E1 契约 FE-2 为准，两处不得分叉 | phper666 | T1 实现前（E1 先行交付管线） | 待办 |
| 归档排除回写共识 | 「归档区任务不进主时间线」为契约实现补充约定（共识 §12 未显式登记），随下次共识修订回写 §12 留痕 | phper666 | 下次共识修订 | 待办 |
...
| 2026-08-22 | 复核修复 | FE/QA 契约复核修复：① MEDIUM——新增 §6 消毒与渲染管线条款（评论条目走 markdown-it+DOMPurify 与 E1 同管线、execution/system 及结构化字段 esc()、禁止裸插）+ XSS 场景 TL7 + 协调事项 E1×T1 评论渲染对齐；② LOW——归档排除标注实现补充约定留痕（协调事项登记回写候选）；③ LOW——新增周视图跨周边界场景 CAL12；④ LOW——新增分页第二页不重不漏场景 PERF4 |
| 2026-08-23 | 复核登记 | 归档排除主时间线复核结论（前次复核②LOW 留痕项落定）：归档条目不进主时间线视图为实现补充约定，PRD 未显式约束，实现行为正确；处理：登记在案为实现约定，FR 条文不改 |
...
- 示例与错误场景：PASS（27 个联调场景 TL1~TL7/CAL1~CAL12/P1~P4/PERF1~PERF4 含成功/失败/边界/性能/XSS 安全；原稿计数 23 有误，实为 24，复核后 27）
- 安全与敏感字段：PASS（零新增 IPC 面；数据仅存 userData + renderer localStorage，不触 DSH_HOME；§6 消毒条款覆盖新增渲染站点 + TL7 XSS 场景）
- 链接与格式：PASS
