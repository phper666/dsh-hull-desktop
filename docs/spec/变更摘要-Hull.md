...
- 类型：共识小版本（实现核验发现）
- 内容：CON-R-backup-001 白名单 skills 项补 `skills/disabled/` 实体目录——原白名单只含 disabled.json / trash.json / trash/，禁用实体（`d_<uuid>/`）未入包 → 换机恢复后禁用记录指向的实体不可恢复（且 merge 侧"实体缺失补拷"路径悬空）；§4.1 表同步。
- 影响：scope parts / 校验守卫清单 / 实体缺失标注三处同源（备份、校验、合并）；契约/设计与测试同步登记
- 下一步：B5 测试与交付核验同步（含 N1/N2/N3 回滚修复后的第三次复验）

## 2026-09-17 备份共识 v1.1——扫描 Q-077~Q-100 全数闭环 + 合并模式（用户决策）

- 类型：共识修订（三角色扫描闭环回写 + 用户决策扩范围）
- 内容：① 关键回写——替换单位改「白名单项集逐项」（Q-077，非整目录：dsh/dsh-previous/node/corepack/Partitions/Singleton* 不动，恢复无需重装 dsh、`.bak` 仅数百 KB）；白名单 9→7 项（Q-089，排除 Partitions/shell 视图记忆与 workflows/runs.json 运行记录）；执行时机 = 运行期只校验 + 写 `<userData>/.restore/pending.json`、启动早期执行（Q-078/079）；notesDir 重定位改「启动后数据卡提示重选」（Q-095，恢复期不弹窗防阻塞启动）；恢复结果跨启动可见（Q-094，`.restore/result.json`）；skills 路径跨机失效 → 标注「路径不存在」+ 按钮禁用，不自动清洗（Q-084）；与更新通道互斥（Q-088）；② **新增合并模式（merge）**（用户决策 2026-09-17）——v1 支持 replace/merge 双模式：看板复用 B5 merge（冲突板重 id + 引用重映射 + 追加）、笔记文件级保留双方（同名不同内容落「原名 (恢复冲突 <ts>).md」）、设置字段级（包内覆盖同名键/本地独有保留）、工作流 id 冲突重 id 追加、通知追加去重、skills 索引并集、dismiss 并集；总原则不静默覆盖、冲突以结果清单呈现；③ 新增 CON-R-backup-011~016（合并模式/中间产物与失败清理/结果可见性/IPC 与 e2e 注入/UI 交互与错误码/测试与验收）；④ U-6/U-7 登记（视图记忆与合并冲突可视化、skills 路径重映射，均 v2）
- 影响：契约需覆盖双模式与按类合并语义；实现新增 restore 启动期执行器 + merge 引擎 + 结果可见链路；测试新增合并场景用例
- 下一步：子需求拆解（Gate B，含 merge）→ 契约 → 判级/设计 → 实现管道
- 关联：docs/spec/共识-Hull桌面壳-备份.md v1.1 · 扫描 24 条 Q-items（飞书 q-item 清单）全部闭环
...
- 内容：新增 CON-R-notes-015——**仅空目录可删**（无子目录且无笔记文件），非空拒绝提示「先移空再删」，空目录直接删除不进回收站（无可恢复内容）；删除入口 = 目录行 hover 删除按钮 + 一次确认；CON-R-notes-009 通道集补 `notes:rmdir`
- 影响：N1 契约（+rmdir 通道）、N2 契约（+目录删除 UI）同步修订；实现补 N1 rmdir + N2 hover × + e2e 场景

## 2026-09-11 笔记共识 v1.2——集成期补 notes:mkdir 通道

- 类型：共识小版本（009 通道集补充；实现联调发现）
- 内容：CON-R-notes-009 通道集补 `notes:mkdir`（mkdirSync recursive + pathGuard 域内校验 + 幂等）——§12「+ 新建目录」按钮需主进程建目录能力，契约冻结时遗漏、集成联调补获；N1 契约同步修订（变更记录 v1.1）
- 影响：N1/N2 实现接线（submitNewDir → notes:mkdir）；Q-064 闭环的通道集清单以本条为准

## 2026-09-11 笔记共识 v1.1——扫描 Q-061~076 全数闭环回写

- 类型：共识修订（Q-items 闭环回写；规则细化 + 2 条新增）
- 内容：① 新增 CON-R-notes-013 路径安全（resolve 域内校验，对齐 skills-007）/ CON-R-notes-014 标题与文件名（frontmatter title 单源、文件名稳定、手动重命名可选——业界惯例外置调研背书）；② 修订 8 条：002 保存冲突分流（mtime 乐观锁，改动三选/删除二选）、005 frontmatter 回写保真（key 级/未知键保留/解析失败头部注入）、006 就绪态中性占位、007 回收站（固定 userData 独立 notes.dir + 隐藏目录排除 + restore 冲突不覆盖 + TTL 细则）、008 自动保存（debounce 2s + flush + Cmd+S，业界笔记应用惯例）、009 通道集闭合 + indexChanged 推送、010 切换前置脏处理、012 验收口径具体化（300 篇种子/首行 <2s/搜索语义）；③ §2/§4/§7/§9/§12/§13 补录（版本历史排除、路径复制、title 字段、常量细则、回收站入口/新建目录/删除确认/视图边界、watch 参数）；④ 契约复核修正：CON-R-notes-011 措辞（view 态序按 shell 现状，非「4→5」计数）
- 影响：Q-061~076 全部 closed；契约阶段可直接引用 v1.1；无 P0 阻断残留
- 闭环：16 条 Q-items 评论答复 + 移 Done（飞书 q-item 清单）
...
- 内容：①通知中心行点击=原地手风琴详情（完整步骤日志+触发/起止绝对时间+总耗时），再点收起；②详情内「查看工作流」显式跳转 → scrollIntoView 居中 + 卡片 flash 描边 1.6s（行点击不再直接跳转）；③工作流页全局「最近运行」段移除（与中心同源冗余，历史归中心 50 条，卡片保留上次运行徽标）
- 核验：notifs e2e 断言重写全绿（展开/收起/步骤/flash/移除）+ 全量回归（2 flake 重跑绿）+ 单测 877 绿
- 视觉修订（同日反馈）：状态/触发列去 pill 改「色点+文本」修列对齐（根因=pill 内边距 vs 表头纯文本）、详情缩进对齐工作流列
- commits：39b1b46 / 8dccb13 → merge（本条随 merge 入 main）
