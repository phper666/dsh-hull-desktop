...
## 验收修复轮 4（2026-09-02，用户四轮反馈「弹框再大+颜色/UI 优化」→ commit 6a22e59）

| # | 反馈 | 修复 |
|:--|:-----|:-----|
| 1 | 弹框再大点 | 近全屏 `min(96vw,1400px) × calc(100vh - 24px)`，上下 12px 呼吸边 |
| 2 | 颜色/UI 优化 | 节点卡暗色三级阶梯底色+边框提档；hover/选中双环 accent；辉光去刺眼（14→10px）；边线语义着色分寸（默认弱化/语义态全强度）；左栏与图区分隔；图例胶囊化；light 覆盖块前移修级联 bug（亮色下语义边被盖灰） |

验证：unit 23/23 · e2e 1/1 · tsc 干净。待用户五轮验收。

## 验收修复轮 5（2026-09-02，用户五轮反馈「按钮语义/全屏/自定义缩放」→ commit 09241e8）

| # | 反馈 | 修复 |
|:--|:-----|:-----|
| 1 | 叉号左边按钮干啥的？点击会缩小？ | 工具条语义化：−(缩小)/倍率/＋(放大)/⛶(适配窗口)/⤢(全屏)，全中文 tooltip；「适配」改 ⛶ 图形语义不再与 ± 混排误读 |
| 2 | 没有全屏按钮 | ⤢ 全屏切换：100vw×100vh 零呼吸边 + 平滑过渡；ESC 先退全屏再关弹框（优先级理清） |
| 3 | 百分比不能自定义 | 点倍率 → inline input（40%~300% 钳制、视口中心锚、非法回退）；输入内 ESC 仅退出编辑 |

验证：unit 23/23 · e2e 1/1 · tsc 干净。待用户六轮验收。
