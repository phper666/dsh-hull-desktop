...
> 版本：v1.4 · 更新：2026-08-20 · 维护者：phper666（PM） · 状态：已发布
...
- **本版本变更**：v1.4 归档机制 + 删除规则 + 视图切换 + B1 复核决策。① **决策 1 归档功能 M2 加**（B1/B2 范围）：Done 列 ticket 可归档 → 归档区（只读视图）；归档区可恢复/彻底删除；Task 增 `archivedAt` + `archivedFromColumnId`；② **决策 2 删除规则（P1-E）**：删看板有 ticket（含归档）→拒删；无 ticket→可删；最后一个看板→不可删（可改名）；删项目前置需清空全部 ticket；③ **决策 3 视图切换 M2 加**：列表视图（表格/排序/筛选）+ 归档视图（只读/恢复/彻底删除）——同 tasks 数据多视图渲染（B2 UI 层）；时间线/日历视图记录 M2+；④ **B1 契约复核决策（P1-E）**："删除唯一看板→validation-error" 废弃，替换为决策 2 删除规则。新规则 CON-R033（归档与删除规则）；登记 Q-030（归档与删除规则，已定案）。
- **历史变更摘要**：（v1.3 三角色扫描 4 P2 回写 + 2 决策——Q-026 心跳超时/Q-027 隐藏列/Q-028 agent 评论只读/Q-029 500ms 测量口径；多项目看板提前进 M2；实时协作记录 M2+。v1.2 三角色扫描 10 P1 回写 + 并行进 M2——O-9 变更 + maxParallelTasks 5→3。v1.1 三角色扫描 3 P0 回写——executionStatus 双轨/dependencies/selfCheck；CON-R030 多 agent 派发。v1.0 为首次建立——登记规则 CON-R017~CON-R029；未决项 U-001~U-003。）

## 2. 文档结构总览

- **覆盖**：M2 任务看板全部业务面——看板（**多项目看板：boards[] 顶层，创建/切换/独立列配置**）/列/卡片/子任务/执行模式（manual/auto）/执行通道（ExecutionProvider/ACP/插件/CLI）/评论时间线/人工干预（暂停/取消/重试/AC 修订/手动完成）/自定义列/人工拖拽语义/**归档功能（决策 1，Done 可归档 + 恢复/彻底删除）**/**视图切换（决策 3：看板/列表/归档）**。
- **适用范围**：仅 Hull 壳内看板业务（规划/跟踪/持久化/执行编排）；**不覆盖** dsh 官方业务（agent 内部行为、官方 UI、会话内容）——那些是 dsh 的领域，Hull 只做容器与执行通道客户端。
- **不做事项（M2 明确排除）**：多设备同步（O-4 定案，P2 导出/导入为过渡）；看板数据加密（O-3 定案，同步/云协作场景再评估 SQLite）；插件独立发布（O-5 定案，随壳分发）；任务级指定 agent/模型（O-10 定案，agentSpec 留位功能排后）；并行依赖图可视化（P2 遗留待办）；**时间线/日历视图（甘特/日历，M2+ 记录，见 §15.3）**；多人协作/权限（**实时协作分享记录为 M2+**，见 §15.3）。
...
| archivedAt | 归档时间（决策 1） | 否 | 系统 | 空=未归档；非空=在归档区 | 无 |
| archivedFromColumnId | 归档前所在列（决策 1） | 否 | 系统 | 恢复用（回原列或 Done） | 无 |
...
| CON-R033 | 归档与删除规则（决策 2/P1-E）：Done 列 ticket 可归档（Task 记 archivedAt + archivedFromColumnId）；归档区 ticket 可恢复（回 Done 或原列）/彻底删除；删除看板有 ticket（含归档）→拒删、无 ticket→可删、最后一个看板→不可删（可改名）；删除项目前置需先清空全部 ticket（含归档区） | 共识 v1.4 | 生效 | 新增 |
...
> PRD O-1~O-11 已全部定案（见 PRD §11.1），不重复登记；此处仅登记 PRD §12 遗留待办（下期承接，文档头"遗留待办"清单同步）。Q-013~Q-030 为扫描/复核问题（载体：飞书 q-item 清单 `dsh-hull-desktop-q-item`）；Q-013~Q-030 已全部确认闭环。
...
| 页面/组件 | 角色 | 功能 | 权限 | 数据范围 |
|:----------|:-----|:-----|:-----|:---------|
| 看板主界面 | 用户 | 左侧 Hull 导航（dsh web/设置/升级/任务看板）+ 右侧看板视图；**多项目看板（决策 1）：看板切换器/创建看板按钮，各看板独立列+任务**；**视图切换器（决策 3）：看板视图 / 列表视图（表格，排序/筛选）/ 归档视图（只读）——同 tasks 数据多视图渲染（B2 UI 层，不改数据模型）**；工具栏（搜索/优先级筛选/状态筛选/标签筛选/清除/管理列/新建任务）；列（300px，列头色带=列颜色，卡片数徽标）；卡片（左侧色条=优先级、模式徽标、父引用徽标、标签、进度条+展开子任务列表、截止/负责人、执行状态徽标、←/→/agent 按钮） | 全量 | 看板数据 |
| 归档区视图 | 用户 | 只读视图：已归档 ticket 列表（Done 归档）；**恢复**（回 Done 或原列 archivedFromColumnId）/ **彻底删除**（purgeTask，二次确认） | 恢复/彻底删除需确认 | 归档区 ticket（archivedAt 非空） |
...
> **归档操作（决策 1/2）**：Done 列 ticket 可归档 → 记 `archivedAt` + `archivedFromColumnId` → 移入归档区（只读）；归档区 ticket 可恢复（回 Done 或原列，清 archivedAt）/ 彻底删除（purgeTask，级联清附件/executions log，二次确认）。
...
| 版本 | 日期 | 变更摘要条目 | 说明 |
|:-----|:-----|:-------------|:-----|
| v1.4 | 2026-08-20 | 变更摘要-M2看板.md 已登记（2026-08-20） | 归档机制 M2 加（archivedAt/archivedFromColumnId + 归档区恢复/彻底删除）+ 删除规则（CON-R033）+ 视图切换（看板/列表/归档）+ B1 复核决策（P1-E 删除规则替换） |
...
| **时间线/日历视图** | M2+ | 甘特/日历视图（中复杂度）；M2 已有列表/归档视图（决策 3），时间线/日历记录后置 |
| 多项目看板 | **已提前进 M2**（决策 1） | 原 P2（FR-11 多看板）提前；boards[] 顶层 + 创建/切换/独立列，见 §7.0/§12 |
| 归档功能 | **M2 加**（决策 1） | Done 可归档 + 恢复/彻底删除；见 §7.1/§12/CON-R033 |
