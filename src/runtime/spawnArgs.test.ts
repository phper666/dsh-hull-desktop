...
test('buildSpawnArgv：node 在前，--expose-internals 在脚本名前，参数齐备', () => {
...
test('buildDshArgv：不含 node 自身，web 子命令与参数齐备', () => {
...
test('DSH_CLI_SIGNATURE：含三段签名，matchesDshSignature 校验', () => {
  ...
  ok(matchesDshSignature('node --expose-internals /dsh/bin/dsh web --host 127.0.0.1 --port 0'));
