...
> 交付核验（2026-09-21）对照本方案逐项核验。

| 日期 | 项 | 结论 |
|:-----|:---|:-----|
| 2026-09-21 | 模块划分（§1） | 一致（`src/plugins/**` 10 文件落地；IPC 6 通道） |
| 2026-09-21 | 偏离 D1~D5 | 显式记录（§四 of records）：通道计数基线、registry 降级口径、verify 启发式、updatable 死路径、entryId 复合键 |
| 2026-09-21 | 安全设计（§4） | 一致（复合键反查、路径穿越校验、kill 兜底、ensureProfile 接线、白名单 https + 逐字命中） |
| 2026-09-21 | 测试方案（§5） | 一致（unit 1343 / integration 40 / e2e 5 断言 + cold-start 回归） |
