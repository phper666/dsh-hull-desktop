 ...
 * 唯一性（无重复）+ B1 16/B5 2/B3 10 计数 + 前缀规约（kanban:）
 */
import { test } from 'node:test';
import { equal, ok } from 'node:assert/strict';

import { ALL_IPC_CHANNELS, KANBAN_B4_EXEC_IPC_CHANNELS, KANBAN_EXEC_IPC_CHANNELS, KANBAN_IPC_CHANNELS } from './ipc-channels';

test('白名单计数：B1 16 + B5 2 = 18，B3 10，B4 4，共面 32', () => {
  equal(KANBAN_IPC_CHANNELS.length, 18, 'B1 16 数据原语 + B5 2 导出/导入');
  equal(KANBAN_EXEC_IPC_CHANNELS.length, 10, 'B3 10 执行控制');
  equal(ALL_IPC_CHANNELS.length, 32);
});
...
test('前缀规约：全部 kanban:', () => {
  for (const c of ALL_IPC_CHANNELS) ok(c.startsWith('kanban:'), `前缀 ${c}`);
});
