...
> 状态：draft（撰写中，待评审）
> 版本：0.1 · 2026-08-22
> 事实源：共识 `docs/spec/共识-Hull桌面壳-M1.md` v1.6 §14.1-R；契约 `docs/api/feishu-s6-m1-api-contract.md` v0.4 / `docs/api/feishu-s3-m1-api-contract.md` v0.3 / S8（无独立契约文件，以 S8 设计 `docs/design/S8-主窗口形态-m1-design.md` + 共识 §14.1-R 为准）；PRD `docs/prd/2026-08-14-m1-prd.md`
> 判级：复杂 + 安全敏感（共识 v1.6 已定）。理由：三个已发布子需求的 UI 载体统一重构（S6'/S3'/S8'），跨 WindowManager/preload/shell.html/main 多文件联动 + view 状态机 2→4 态扩展；涉 CON-R001（preload 桥合并白名单面扩大）、CON-R002（settings 持久化路径）、CON-R005（升级编排零改动）安全边界，必须方案评审冻结后进实现。
...
**定位**：统一壳内交互模型——把已发布的 M1 壳功能（设置页独立窗口、升级原生 dialog）收进主窗口右侧内容区，壳框架 view 状态机从 2 态扩为 4 态。对已发布期的重构，归 M1 增补段 `m1refactor`，不占新期编号（期编号决策模型：对已发布期的重构归该期增补段，需求 slug `m1refactor`）。

**范围**（共识 v1.6 §14.1-R + 契约冻结）：

- **S6' 设置页迁移**：独立 SettingsWindow → 壳内右侧整页 section（重渲染，删除独立窗口）；壳导航「设置」切换；settings 持久化仍走主进程 `hull:setSettings` 门面（CON-R002 不破）；双升级区块/通用设置/诊断全部可见可操作（D1 已定）
- **S3' 升级面板**：升级面板为独立右侧视图，壳导航「升级」切换；dsh/Hull 双通道确认/进度/失败/回滚/重启安装提示收进壳内统一模式；升级原子性编排逻辑（UpgradeQueue/SwapManager/Updater）零改动（CON-R005 不破）（D4 已定）
- **S8' view 机制扩展**：壳框架 view 状态机扩为 official/board/settings/upgrade 4 态；hull:status payload + nav 高亮回写；托盘入口聚焦主窗口 + 切对应视图（D5 已定）；官方 WebContentsView 零注入结构不变（CON-R001）

**已拍板决策**（2026-08-22 用户）：D1 设置页整页 section 收右侧 / D4 独立 upgrade 视图 / D5 保留托盘补充入口（聚焦主窗口 + 切视图）。
...
- **选 A 理由**：
  - board 先例：`placeholder:board` 已是占位并集成员，nav「任务看板」→ `showPlaceholder('board')` → section#board 显示。settings/upgrade 完全同构，无新机制。
  - 契约零破坏：hull:status payload 的 `view` 字段仍是 `official | placeholder:*` 字符串并集，S8 设计的 `setView(payload.view)`（`view.startsWith('placeholder:')` → 显示对应 section）继续工作，只是 sections 数组扩两个。
  - renderer 无 view 控制通道不变式保持：section 显示仍完全由主进程 view 字段驱动。
  - 高亮回写依赖 view 字段：`placeholder:settings` → nav-settings active、`placeholder:upgrade` → nav-upgrade active，映射集中一处（§4.4）。
- **影响**：hull:status payload 契约扩展（不破坏既有取值，只加 2 个新值）；`sections` 数组 +2；showPlaceholder 调用点新增两处。

### D6 双通道确认流收收范围：含 Hull 自更新（S5 面），联动 S5

- **方案 A（选中）：含 Hull 通道**——upgrade 视图统一承载 dsh + Hull 双通道确认/进度/失败/回滚/重启安装提示。
- **方案 B（否决）：只收 dsh 通道**——Hull 确认流仍走设置页内嵌 modal（S6 现状）。
- **选 A 理由**：
  - 契约 S3 v0.3 已明示「dsh/Hull 双通道确认/进度/失败/回滚/重启安装提示收进壳内统一模式」（§范围）；共识 §14.1-R S3' 验收同样写「dsh/Hull 双通道」。只收 dsh 违反冻结契约。
  - 一致性：native dialog 全部移除（main/index.ts 约 10 处 `dialog.showMessageBox` 调用归零），Hull 通道若留在设置页 modal 会出现「两种确认 UI 并存」，违背统一壳内交互模型目标。
  - 边界（不越权）：Hull 通道只收 UI 载体（确认/进度/失败/重启安装提示），HullUpdater 编排逻辑（download/installAndRestart/autoCheck 门控）零改动；DismissStore 分通道键 `{ dsh?, hull? }`（S5 已实现）保持——upgrade 视图「稍后再说」分别调 `dismissDshUpdate` / `dismissHullUpdate`。
  - 联动 S5 成本低：Hull 通道 UI 已存在于 settings.html（DOM modal 确认流 + `getHullUpdateStatus` 250ms 轮询），迁移到 shell.html upgrade section 是搬移 + 合并，无新编排。
- **影响**：S5 面（Hull 自更新 UI 载体）本方案一并覆盖；S5 契约（feishu-s5-m1-api-contract.md）补变更传播注记；设置页 Hull 区块「检查 Hull 更新」按钮语义保留（在 settings section 内），但确认/进度流统一走 upgrade 视图（见 §3 职责划分，双入口说明）。
...
| D4 | 独立 upgrade 视图 | shell.html 新增 `section#upgrade`，壳导航「升级」切换 |
| D5 | 保留托盘补充入口 | 托盘「Hull 设置」→ 聚焦主窗口 + 切 settings 视图；「检查 dsh 更新」→ 聚焦主窗口 + 切 upgrade 视图；升级中禁用（S5 busy 数据源不变） |
...
| 文件 | 动作 | 职责 |
|:-----|:-----|:-----|
| `src/window/WindowManager.ts` | 重构 | view 状态机扩占位并集（PlaceholderView + settings/upgrade）；`showSettings()` / `showUpgrade()` 便捷方法（封装 `showPlaceholder` + 推送）；hull:status payload 不变（view 字段新取值自动透传） |
| `src/preload/index.ts` | 扩展 | hull 桥并入 settings.ts 15 方法（getSettings/setSettings + 双通道升级全套 + S4 通道 + Hull 全套 + 诊断）；**收敛 checkDshUpdate 命名冲突**（§4.3） |
...
| `tests/e2e/cold-start.spec.ts` / `helpers.ts` | 小改 | helpers 定位（如需 settings 页面 URL 断言移除）；cold-start 不受影响 |
| `tests/e2e/kanban.spec.ts` | 不变 | board 视图语义不变 |
| 契约 S6/S3 | 已更新 | v0.4 / v0.3（M1-重构变更传播） |
| 契约 S5 | 变更传播 | Hull 通道 UI 载体收进 upgrade 视图注记 |
...
### 4.1 view 状态机 4 态迁移矩阵（S8'）

`ViewState = 'official' | PlaceholderView`，`PlaceholderView = 'placeholder:starting' | 'placeholder:installing' | 'placeholder:failed' | 'placeholder:not-installed' | 'placeholder:board' | 'placeholder:settings' | 'placeholder:upgrade'`。
...
| nav 设置 / 托盘 Hull 设置 | `showSettings()` → `showPlaceholder('settings')` | 任意 → `placeholder:settings`（新） |
| nav 升级 / 托盘检查 dsh 更新 | `showUpgrade()` → `showPlaceholder('upgrade')` | 任意 → `placeholder:upgrade`（新） |
| 离开 official | showPlaceholder 任意模式 | `official` → `placeholder:*` 时置 `officialDirty=true`（既有语义，settings/upgrade 同走） |

**迁移不变量**（S8 既有，S8' 保持）：离开 `official` 会置 `officialDirty`，下次 Ready 同 view 再 loadURL；`placeholder:board/settings/upgrade` 是用户主动切视图，不改变 official 加载状态（离开官方 UI 后待机，不销毁 officialView）。**主进程单一事实源**：右侧内容区显示完全由 hull:status 的 `view` 字段驱动，renderer 无 view 控制通道不变式保持。
...
const navActiveByView = {
  ...
  'placeholder:upgrade': 'nav-upgrade',
};
...
| 调用点 | 现状 | S8' 后 |
|:-----|:-----|:-----|
| runCheck 发现新版本确认 | `showMessageBox [立即升级/稍后再说]` | 切 upgrade 视图（若不在）+ 渲染确认卡片（版本/变更/「立即升级/稍后再说」）；稍后再说 → `dismissDshUpdate` + `dismissStore.dismissToday('dsh')` |
...
**设置 section 与 upgrade 视图职责分工（D6 边界）**：
...
- 「检查 dsh 更新」→ 聚焦主窗口 + `showUpgrade()` + 触发 check。
...
├── src/
│   ├── window/
│   │   ├── WindowManager.ts          # 改：view 状态机扩 2 态（settings/upgrade）+ showSettings/showUpgrade
│   │   └── SettingsWindow.ts         # 删（D1：独立设置窗口移除）
│   ├── preload/
│   │   ├── index.ts                  # 改：hull 桥并入 settings 15 方法 + checkDshUpdate 收敛
...
└── tests/
    ├── e2e/
    │   ├── helpers.ts                # 小改：定位适配
    │   ├── settings.spec.ts          # 改：独立窗口 → section#settings 断言
    │   ├── upgrade.spec.ts           # 改：原生 dialog → section#upgrade 断言
