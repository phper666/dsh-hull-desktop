...
- 内容：① 新增 CON-R-notes-013 路径安全（resolve 域内校验，对齐 skills-007）/ CON-R-notes-014 标题与文件名（frontmatter title 单源、文件名稳定、手动重命名可选——业界惯例外置调研背书）；② 修订 8 条：002 保存冲突分流（mtime 乐观锁，改动三选/删除二选）、005 frontmatter 回写保真（key 级/未知键保留/解析失败头部注入）、006 就绪态中性占位、007 回收站（固定 userData 独立 notes.dir + 隐藏目录排除 + restore 冲突不覆盖 + TTL 细则）、008 自动保存（debounce 2s + flush + Cmd+S，业界笔记应用惯例）、009 通道集闭合 + indexChanged 推送、010 切换前置脏处理、012 验收口径具体化（300 篇种子/首行 <2s/搜索语义）；③ §2/§4/§7/§9/§12/§13 补录（版本历史排除、路径复制、title 字段、常量细则、回收站入口/新建目录/删除确认/视图边界、watch 参数）
