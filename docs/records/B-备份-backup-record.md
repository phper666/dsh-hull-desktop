...
### 验收发现（2026-09-18，用户手动验收）

- **现象**：备份后新建笔记 → 替换恢复后笔记从视图消失（数据未丢，在 `.restore/backup-<ts>/`，可找回）。
- **根因**：替换模式恢复到备份时刻状态——备份之后新增/修改的内容被移入预备份目录；确认弹窗警告过弱（只写"先备份到 .restore/backup"）+ 无找回入口。
- **修复**：① 确认弹窗按模式显式警告（替换=备份后新增/修改内容被移出、结果卡给路径可找回；合并=保留双方冲突可见）；② 结果卡新增「打开预备份目录」按钮（复用 `hull:openPath`）；③ e2e 补断言（`[data-bak-open]` 可见，happy path）。
- **次要登记（v2）**：空目录不进包（跨机恢复丢空目录）；恢复前"检测备份后新增内容并提示"。
