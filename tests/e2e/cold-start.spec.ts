...
test.describe('E2E-01 冷启动', () => {
  test('启动 → 壳窗口 + nav 可见 → 官方 view 就绪，总时长 ≤10s（Q-009 权威口径）', async () => {
    ...
    try {
      ...
      // nav 排序（M1-重构 + S1 + tokens/connections/workflows 增量 + P4 插件）：dsh web → 任务看板 → 笔记 → Skills → 插件 → Token 消耗 → 工作台连接 → 工作流 → 通知 → 设置（设置恒最后，无 nav-upgrade）
      const navOrder = await shell.locator('#nav-items .nav-item').evaluateAll((els) => els.map((e) => e.id));
      expect(navOrder).toEqual(['nav-web', 'nav-board', 'nav-notes', 'nav-skills', 'nav-plugin', 'nav-tokens', 'nav-connections', 'nav-workflows', 'nav-notifs', 'nav-settings']);
