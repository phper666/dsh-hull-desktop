...
- **覆盖**：Hull dsh 安装链路全部业务面——两包管理器（npm/pnpm）支持、设置页选择 + settings 持久化、原生依赖自动 rebuild、spawn 跨平台改造（绕开 .bin shim）、取消/错误码一致。
...
| 术语 | 定义 | 出处 |
|:-----|:-----|:-----|
| 包管理器 | 安装 dsh 依赖的工具：npm / pnpm（均 node 生态） | PRD §1 |
...
- pnpm：非交互 approve-builds 或 `pnpm rebuild <pkgs>`；
- 失败告警不阻断安装（可手动补装）。
...
| CON-R-pkgmgr-007 | 错误码映射两包管理器一致（registry-unreachable/npm-install-failed 复用） | PRD FR-5 | 生效 | 稳定 |
...
| 页面/组件 | 角色 | 功能 | 权限 | 数据范围 |
|:----------|:-----|:-----|:-----|:---------|
| 设置页「包管理器」区块 | 用户 | 三选一选择（npm/pnpm），当前高亮，切换即时生效持久化 | 全量 | 壳全局 |
...
|:--|:-------|:-------------------|:---------|:-----|:---------|:-------|
| P1 | 包管理器执行器抽象（npm/pnpm 两实现） | 两包管理器均能装 dsh 到 staging；输出进度落盘；取消杀子进程；**错误码解析按包管理器适配**（pnpm 错误格式不同于 npm 的 `npm error code`，registry-unreachable 判定需分别处理）；**取消杀完整进程树**（pnpm 可能有 store/worker 子进程） | CON-R-pkgmgr-001/002/006/007 | 无 | PRD FR-2/FR-5 | t100086 |
...
| P3 | settings.packageManager + 设置页选择 + 原生依赖自动 rebuild | 设置页二选一持久化重启保持；pnpm 装完自动 rebuild 原生依赖 | CON-R-pkgmgr-001/003/008 | P1 | PRD FR-1/FR-3 | t100088 |
