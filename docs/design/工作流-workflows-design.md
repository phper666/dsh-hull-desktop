...
### 9.5 交互修订（2026-09-03 用户三项确认，判级：简单 → 轻量实现）

1. **工作流页全局「最近运行」段移除**：与通知中心同源冗余；卡片保留「上次成功/失败」徽标，历史统一归通知中心（50 条）。
2. **通知中心行点击 = 原地展开详情（手风琴）**：完整步骤日志（每步 ✓/✗ + 类型 + 消息 + 耗时）+ 触发来源 + 起止绝对时间 + 总耗时；再点收起。
3. **跳转改显式动作**：详情内「查看工作流」按钮 → 切工作流视图 + **高亮定位**（scrollIntoView 居中 + 卡片 flash 描边 ~1.6s）；行点击不再直接跳转。
