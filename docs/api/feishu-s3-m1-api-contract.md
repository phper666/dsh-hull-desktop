...
- **UI 载体（M1-重构 S3'，v0.4 修订）**：升级确认/进度/失败提示/回滚/重启安装提示从原生 dialog/DOM modal 收进壳内统一模式——**并入设置页升级区块**（v0.3 曾定独立 upgrade 视图，v0.4 按用户目验反馈修订为并入设置，删独立视图）；升级原子性编排逻辑（UpgradeQueue/SwapManager/Updater）零改动（CON-R005 不破）
...
> 时序注记（A2）：#1/#4 调用方设置页归 S6 接线——S3 先行 main/托盘入口 + 原生 dialog 交付；**M1-重构 S3' 后（v0.4 修订）**升级确认/进度/失败提示收进设置页升级区块（v0.3 曾定独立 upgrade 视图，v0.4 用户目验反馈并入设置），原生 dialog 移除；按钮禁用态 = canRollback() 数据源不变。
...
- 2026-08-22：M1-重构迭代调整（S3'，共识 v1.7）：升级 UI 载体从独立 upgrade 视图**并入设置页升级区块**（用户目验反馈修订 D4）；删独立视图；确认/进度/失败/回滚/重启安装提示设置页壳内呈现；升级原子性编排逻辑零改动（CON-R005 不破）；#1~#4 调用方改设置页升级区块。升 v0.4
